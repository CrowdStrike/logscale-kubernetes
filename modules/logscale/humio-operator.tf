...
resource "null_resource" "humio_operator_replica_patch" {
  ...
  provisioner "local-exec" {
    command = var.kubectl_context != "" ? "kubectl --context=${var.kubectl_context} patch deployment ${helm_release.humio_operator.name} -n ${var.k8s_namespace_prefix} -p '{\"spec\":{\"replicas\":0}}'" : "kubectl patch deployment ${helm_release.humio_operator.name} -n ${var.k8s_namespace_prefix} -p '{\"spec\":{\"replicas\":0}}'"
  }
